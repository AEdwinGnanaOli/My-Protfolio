...
const Project = () => {
  ...
  {items.map((item) => (
    <motion.div
      ...
      className="item-container bg-gray-800 rounded-lg shadow-lg p-3 flex flex-col justify-between hover:shadow-primary duration-500"
    >
      <div className="item h-full">
        <img
          className="rounded-lg h-full w-full"
          src={item.mainImage || "/placeholder-image.jpg"}
          alt={item.title || "Default Item"}
          loading="lazy"
        />
        <div className="overlay m-3">
          <h3 className="text-2xl text-primary font-semibold">
          ...
          <Link to={`/project/${item.id}`} className="mt-3 inline-block">
            <button
              className="btn btn-sm border-2 border-transparent bg-primary hover:bg-transparent text-white hover:border-primary duration-500"
              aria-label={`See details for ${item.title || "Untitled Project"}`}
            >
              See Details
            </button>
          </Link>
